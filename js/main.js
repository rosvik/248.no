...
function myLoop(s, f, action) {
    ...
    function need_insert(i, x, y) {
        if (
            x < art_line_lenght + dx &&
            x > dx && // Check if [x,y] in range from
            y < art_line_count + dy &&
            y >= dy
        ) {
            // [x+dx, y+dy] to [all+dx, alc+dy]
...
function setLetter(i, x, y) {
    // Clear cursor fill
    if (prev_char_element != null)
        prev_char_element.css("background-color", "transparent");
    var char_element = $(".char" + i);
